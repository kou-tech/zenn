...
同値分割法は、入力を代表する値のグループに分けてテストを行う技法です。

例として、注文日から3日以内であればキャンセル可能かを判断する関数を考えます。
...
:::message
グループ分けの判断や代表値の選び方は、テスト分析やテスト設計の段階で決定されるため、ここでは詳細な説明は省略します。
:::
...
それぞれのグループから代表値を一つ選びテストすることで、他のグループに属する値も同様に処理されると仮定します。有効な値と無効な値を含むグループに区別することが重要です。
この方法により、すべての値をテストすることなく、効率的にテストを行うことができます。
同値分割法は、入力値の組み合わせが少なく、グループ分けが可能な場合に特に有効です。
