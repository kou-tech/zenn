...
```php
$metadata['https://[App Serviceの規定のドメイン]/simplesaml/saml2/idp/metadata.php'] = [
...
```
...
「Test」タブからexample-userpassを選択し、ユーザー名とパスワードを入力してログインテストを実行できます。
...
また、SimpleSAMLphpのキャッシュディレクトリの権限設定がなく、エラーが発生する場合があります。

そのため、以下のコマンドを実行してキャッシュディレクトリの権限を適切に設定する必要があります。
