...
スキーマ駆動開発の観点から、個人的な意見として以下のように整理する。

| 書いてよいもの | 書いてはいけないもの |
|----------------|----------------------|
| データのフォーマット変換（日付の形式変更など） | ビジネスロジック（金額計算、集計など） |
| アクセサで定義した値の参照 | 条件に基づく複雑な判断 |
| ネストしたリソースへの変換 | データベースクエリの実行 |
| | 外部APIとの通信 |
| | ソート・フィルター |
| | `when()`等による動的プロパティ |

このように整理する理由は、Resource の責務を「変換ロジック」というシンプルなものに留めるためである。文字列結合などのロジックはモデルのアクセサに委ね、Resource は受け取ったデータを API レスポンス形式に変換することだけに集中させる。これにより保守性が向上し、各クラスの責務が明確になる。
